...
Describe 'Copilot UserPromptSubmit required-memory injection' {
    BeforeAll {
        $script:PluginRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).ProviderPath
        $script:MemoryContext = @(
            (Join-Path $script:PluginRoot 'hooks/scripts/memory-context.ps1')
            (Join-Path $script:PluginRoot 'lib/memory-context.ps1')
    ...
    BeforeEach {
        $script:SavedEnv = @{
            ...
            COPILOT_PLUGIN_ROOT = $env:COPILOT_PLUGIN_ROOT
        }
        ...
        $env:COPILOT_PLUGIN_ROOT = $script:PluginRoot
...
sessionId: Copilot-20260714T000000Z-plugin-session
agent: Copilot
status: verified
...
"@.Trim() + "`n")
    ...
    It 'loads the host descriptor and maps memory_* aliases to workflow.memory.*' {
        ...
        $descriptor.host | Should -Be 'copilot'
