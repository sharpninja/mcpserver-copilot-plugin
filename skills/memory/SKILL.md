...
Use the Copilot plugin memory surface. Do not invent memories. Prefer the plugin REPL/workflow wrapper over local files. This plugin does not register native MCP `memory_*` tools; those names are aliases for `workflow.memory.*` methods invoked through `lib/repl-invoke.ps1` or `skills/memory/scripts/invoke.ps1`.
